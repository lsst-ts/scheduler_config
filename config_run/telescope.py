import lsst.ts.schedulerConfig.instrument.telescope
assert type(config)==lsst.ts.schedulerConfig.instrument.telescope.Telescope, 'config is of type %s.%s instead of lsst.sims.ocs.configuration.instrument.telescope.Telescope' % (type(config).__module__, type(config).__name__)
